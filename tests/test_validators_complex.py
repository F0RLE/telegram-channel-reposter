...
class TestComplexTemperatureValidation:
    ...
    def test_valid_temperature_range(self):
        ...
        valid_temps = [0.0, 0.1, 0.5, 0.7, 1.0, 0.99]
    ...
    def test_invalid_temperature_values(self):
        ...
        invalid_temps = [-0.1, 1.1, 2.0, -1.0, None, "0.5"]
...
class TestComplexTextSanitization:
    ...
    def test_sanitize_various_texts(self):
        ...
        test_cases = [
            ...
            ("Text with\x00null\x00chars", "Text with null chars"),
        ]
        for input_text, expected in test_cases:
            result = sanitize_text(input_text)
            assert result == expected, f"Sanitization failed for: {input_text[:20]}"
    
    def test_sanitize_long_text(self):
        """Test sanitization of long text"""
        long_text = "A" * 10000
        result = sanitize_text(long_text)
        assert len(result) <= 5000, "Long text should be truncated"
        assert result == "A" * 5000, "Long text should be truncated correctly"
