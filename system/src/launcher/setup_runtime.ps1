...
Write-Host "[SETUP] Extracting Tkinter components (this takes a moment)..." -ForegroundColor Yellow
$TempPython = "$TempDir\python_full"
# Install to temp dir to get files
Start-Process -FilePath $Installer -ArgumentList "/quiet", "InstallAllUsers=0", "TargetDir=$TempPython", "Include_tcltk=1", "Include_pip=0", "Include_test=0", "PrependPath=0" -Wait

# 5. Transplant Tkinter Files
Write-Host "[SETUP] Transplanting Tkinter files..." -ForegroundColor Yellow

# Copy tcl folder
Copy-Item -Path "$TempPython\tcl" -Destination "$PythonDir\tcl" -Recurse

# Copy tkinter module
New-Item -ItemType Directory -Path "$PythonDir\Lib" -Force | Out-Null
Copy-Item -Path "$TempPython\Lib\tkinter" -Destination "$PythonDir\Lib\tkinter" -Recurse

# Copy DLLs
Copy-Item -Path "$TempPython\DLLs\_tkinter.pyd" -Destination "$PythonDir\_tkinter.pyd"
Copy-Item -Path "$TempPython\DLLs\tcl86t.dll" -Destination "$PythonDir\tcl86t.dll"
Copy-Item -Path "$TempPython\DLLs\tk86t.dll" -Destination "$PythonDir\tk86t.dll"
