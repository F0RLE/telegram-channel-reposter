...
declare function t(key: string, fallback?: string, params?: Record<string, unknown>): string;
declare function updateState(): Promise<void>;
declare function updateRangeProgress(fieldId?: string): void;
declare function updateTokenCount(): void;
declare function agentLog(level: string, location: string, action: string, data?: Record<string, unknown>): void;
declare function toggleAdZones(): void;
...
window.control = async function (action: string, service: string): Promise<void> {
    try {
...
async function loadSettings(): Promise<void> {
    try {
        ...
        try {
            ...
            if (modsPayload) {
                const items = (modsPayload.items || modsPayload.modules || []) as any[];
                const llmMod = items.find((m: any) => m.id === 'llm');
                const sdMod = items.find((m: any) => m.id === 'sd');
        ...
        const safeCall = (fnName) => {
            ...
            if (typeof (window as any)[fnName] === 'function') (window as any)[fnName]();
        };
        ...
        // General
        (document.getElementById('field-gpu') as HTMLInputElement).checked = data.USE_GPU !== 'false';
        // Default DEBUG_MODE to true if not set (if element exists)
        const debugField = document.getElementById('field-debug');
        if (debugField) {
            (debugField as HTMLInputElement).checked = data.DEBUG_MODE === 'true' || data.DEBUG_MODE === undefined || data.DEBUG_MODE === '';
        }
        ...
        if (llmSys) (llmSys as HTMLTextAreaElement).value = removeQuotes(data.llm_rewrite_system_prompt) || defaultSysPrompt;
        if (llmUser) (llmUser as HTMLTextAreaElement).value = removeQuotes(data.llm_rewrite_user_prompt) || defaultUserPrompt;
        if (llmPositive) (llmPositive as HTMLTextAreaElement).value = removeQuotes(data.llm_positive_prompt) || defaultSdPositive;
        if (llmNegative) (llmNegative as HTMLTextAreaElement).value = removeQuotes(data.llm_negative_prompt) || defaultSdNegative;
        if (llmCliches) (llmCliches as HTMLTextAreaElement).value = removeQuotes(data.llm_rewrite_cliches) || defaultCliches;
        if (llmTemp) (llmTemp as HTMLInputElement).value = data.llm_temp || 0.7;
        if (valLlmTemp) valLlmTemp.innerText = parseFloat(data.llm_temp || 0.7).toFixed(1);
        const ctxValue = data.llm_ctx || 4096;
        if (llmCtx) (llmCtx as HTMLInputElement).value = ctxValue;
        ...
        if (data.MODELS_LLM_DIR) {
            (document.getElementById('field-models-dir') as HTMLInputElement).value = removeQuotes(data.MODELS_LLM_DIR);
        } else {
            ...
            try {
                ...
                if (config.MODELS_LLM_DIR) {
                    (document.getElementById('field-models-dir') as HTMLInputElement).value = removeQuotes(config.MODELS_LLM_DIR);
                }
            } catch (e) { }
        }

        // SD
        (document.getElementById('field-sd-args') as HTMLInputElement).value = removeQuotes(data.SD_ARGS);
        (document.getElementById('field-sd-model-url') as HTMLInputElement).value = removeQuotes(data.SD_MODEL_URL);
        if (data.MODELS_SD_DIR) {
            const sdModelsDirField = document.getElementById('field-sd-models-dir');
            if (sdModelsDirField) {
                (sdModelsDirField as HTMLInputElement).value = removeQuotes(data.MODELS_SD_DIR);
            }
        } else {
            ...
            try {
                ...
                if (sdModelsDirField) {
                    if (config.MODELS_SD_DIR) {
                        (sdModelsDirField as HTMLInputElement).value = removeQuotes(config.MODELS_SD_DIR);
                    } else {
                        ...
                        const defaultPath = config.SD_DIR ?
                            ...
                            '';
                        if (defaultPath) {
                            (sdModelsDirField as HTMLInputElement).value = defaultPath;
                        }
                    }
                }
            } catch (e) { }
        ...
        if (data.sd_steps) {
            (document.getElementById('field-sd-steps') as HTMLInputElement).value = data.sd_steps;
            document.getElementById('val-sd-steps').innerText = data.sd_steps;
            updateRangeProgress('field-sd-steps');
        }
        if (data.sd_cfg) {
            (document.getElementById('field-sd-cfg') as HTMLInputElement).value = data.sd_cfg;
            document.getElementById('val-sd-cfg').innerText = parseFloat(data.sd_cfg).toFixed(1);
            updateRangeProgress('field-sd-cfg');
        }
        if (data.sd_width) (document.getElementById('field-sd-w') as HTMLInputElement).value = data.sd_width;
        if (data.sd_height) (document.getElementById('field-sd-h') as HTMLInputElement).value = data.sd_height;
        if (data.sd_sampler) (document.getElementById('field-sd-sampler') as HTMLInputElement).value = data.sd_sampler;
        if (data.sd_seed) (document.getElementById('field-sd-seed') as HTMLInputElement).value = data.sd_seed;
        (document.getElementById('field-sd-positive') as HTMLTextAreaElement).value = removeQuotes(data.sd_positive_prefix);
        (document.getElementById('field-sd-negative') as HTMLTextAreaElement).value = removeQuotes(data.sd_negative_prompt);

        // ADetailer
        (document.getElementById('field-ad-enabled') as HTMLInputElement).checked = data.ad_enabled;
        if (data.ad_steps) {
            (document.getElementById('field-ad-steps') as HTMLInputElement).value = data.ad_steps;
            document.getElementById('val-ad-steps').innerText = data.ad_steps;
        }
        (document.getElementById('field-ad-face') as HTMLInputElement).checked = data.ad_face;
        (document.getElementById('field-ad-hand') as HTMLInputElement).checked = data.ad_hand;
        (document.getElementById('field-ad-person') as HTMLInputElement).checked = data.ad_person;
        (document.getElementById('field-ad-clothing') as HTMLInputElement).checked = data.ad_clothing;
        if (data.ad_face_conf) {
            (document.getElementById('field-ad-face-conf') as HTMLInputElement).value = data.ad_face_conf;
            document.getElementById('val-ad-face-conf').innerText = parseFloat(data.ad_face_conf).toFixed(2);
        }
        if (data.ad_hand_conf) {
            (document.getElementById('field-ad-hand-conf') as HTMLInputElement).value = data.ad_hand_conf;
            document.getElementById('val-ad-hand-conf').innerText = parseFloat(data.ad_hand_conf).toFixed(2);
        }
        if (data.ad_person_conf) {
            (document.getElementById('field-ad-person-conf') as HTMLInputElement).value = data.ad_person_conf;
            document.getElementById('val-ad-person-conf').innerText = parseFloat(data.ad_person_conf).toFixed(2);
        }
        if (data.ad_clothing_conf) {
            (document.getElementById('field-ad-clothing-conf') as HTMLInputElement).value = data.ad_clothing_conf;
...
async function loadSdModels(): Promise<void> {
    try {
...
window.selectSdModel = async function (name: string): Promise<void> {
    try {
...
window.deleteSdModel = async function (name: string): Promise<void> {
...
window.selectLlmModel = async function (name: string, type: string): Promise<void> {
    try {
...
window.saveLlmPrompts = async function (showNotification: boolean = true): Promise<void> {
    try {
        const prompts = {
            llm_rewrite_system_prompt: { value: (document.getElementById('field-llm-sys') as HTMLTextAreaElement)?.value || '', fieldId: 'llm-sys' },
            llm_rewrite_user_prompt: { value: (document.getElementById('field-llm-user') as HTMLTextAreaElement)?.value || '', fieldId: 'llm-user' },
            llm_positive_prompt: { value: (document.getElementById('field-llm-positive') as HTMLTextAreaElement)?.value || '', fieldId: 'llm-positive' },
            llm_negative_prompt: { value: (document.getElementById('field-llm-negative') as HTMLTextAreaElement)?.value || '', fieldId: 'llm-negative' },
            llm_rewrite_cliches: { value: (document.getElementById('field-llm-cliches') as HTMLTextAreaElement)?.value || '', fieldId: 'llm-cliches' }
        };
...
window.saveSdPrompts = async function (showNotification: boolean = true): Promise<void> {
    try {
        const prompts = {
            sd_positive_prefix: { value: (document.getElementById('field-sd-positive') as HTMLTextAreaElement)?.value || '', fieldId: 'sd-positive' },
            sd_negative_prompt: { value: (document.getElementById('field-sd-negative') as HTMLTextAreaElement)?.value || '', fieldId: 'sd-negative' }
        };
...
function getSaveIndicator(): HTMLDivElement {
    if (!saveIndicator) {
...
function markUnsaved(fieldId: string): void {
...
async function autoSaveField(fieldId: string): Promise<void> {
    const allFields = {
    ...
    try {
        let value = config.isBool ? ((el as HTMLInputElement).checked ? 'true' : 'false') : (el as HTMLInputElement).value;
...
function clearUnsaved(fieldId: string | null): void {
    if (fieldId) {
        delete unsavedChanges[fieldId];
    } else {
        unsavedChanges = {};
...
window.saveAllChanges = async function (): Promise<void> {
    // Save all prompts
    ...
    for (const [fieldId, config] of Object.entries(allFields)) {
        if (unsavedChanges[fieldId]) {
            const el = document.getElementById('field-' + fieldId);
            if (el) {
                let value = (config as any).isBool ? ((el as HTMLInputElement).checked ? 'true' : 'false') : (el as HTMLInputElement).value;
...
async function saveAllChanges(): Promise<void> {
    ...
    for (const fieldId of Object.keys(unsavedChanges)) {
        ...
        let value = (field as any).isCheckbox ? ((el as HTMLInputElement).checked ? 'true' : 'false') : (el as HTMLInputElement).value;
...
async function saveSetting(key: string, value: unknown, isJson: boolean = false, showNotification: boolean = false, fieldId: string | null = null): Promise<void> {
    if (typeof value === 'number') value = parseFloat(String(value));
